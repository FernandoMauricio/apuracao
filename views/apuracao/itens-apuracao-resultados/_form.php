    ...
    <?php foreach ($temas as $i => $tema) : ?>
        ...
        <div id="collapse<?=$i+1?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="collapse<?=$i+1?>">
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-12">
                        <?= $form->field($model, "[{$i}]item_apure_acaorealizada")->textarea(['rows' => 3]) ?>
                    </div>
                </div>

                <div class="row">
                    <div class="col-md-4">
                        <?= $form->field($model, "[{$i}]item_apure_local")->textarea(['rows' => 3]) ?>
                    </div>

                    <div class="col-md-4">
                        <?= $form->field($model, "[{$i}]item_apure_datarealizacao")->textarea(['maxlength' => true, 'rows' => 3]) ?>
                    </div>

                    <div class="col-md-4">
                        <?= $form->field($model, "[{$i}]item_apure_motivo")->textarea(['maxlength' => true, 'rows' => 3]) ?>
                    </div>
                ...
                <div class="row">
                    <div class="col-md-3">
                        <?= $form->field($model, "[{$i}]item_apure_publico")->textarea(['maxlength' => true, 'rows' => 3]) ?>
                    </div>

                    <div class="col-md-3">
                        <?= $form->field($model, "[{$i}]item_apure_qntpessoas")->textarea(['maxlength' => true, 'rows' => 3]) ?>
                    </div>

                    <div class="col-md-3">
                        <?= $form->field($model, "[{$i}]item_apure_parceiros")->textarea(['maxlength' => true, 'rows' => 3]) ?>
                    </div>
